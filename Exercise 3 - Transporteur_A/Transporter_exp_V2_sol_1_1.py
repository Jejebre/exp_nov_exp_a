...
def prox():
    ...
    if mode == NORMAL:
        nf_leds_top(0, 32, 0)  # Green - Moving Forward
        motor_left_target = 250  # Move forward
        motor_right_target = 250

        # Check for obstacles
        if prox_horizontal[2] > 2500 or prox_horizontal[3] > 2500:  # Wall detected
            mode = WALLS

    elif mode == WALLS:
        nf_leds_top(32, 0, 0)  # Red - Moving Backward
        motor_left_target = -250  # Move backward
        motor_right_target = -250

        # Check for line while moving backward
        if prox_ground_delta[0] < 800 or prox_ground_delta[2] < 800:  # Line detected
            mode = LINE
            motor_left_target = 0  # Stop backward movement
            motor_right_target = 0
            call delay(500)  # Pause for a moment
...
def timer0():
    ...
    if mode == LINE:
        motor_left_target = 250  # Move forward again
        motor_right_target = 250
        nf_leds_top(32, 32, 0)  # Orange - Moving Forward
        mode = NORMAL  # Reset to normal mode

@onevent
def buttons():
    global motor_left_target, motor_right_target, mode
    if button_center:  # If the center button is pressed
        motor_left_target = 0
        motor_right_target = 0
        mode = STOP
